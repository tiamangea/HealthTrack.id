...
const generateWorkers = () => {
  ...
  return names.map((name, i) => {
    ...
    return {
      ...
      name: name,
      role: i === 0 ? 'Driller (Offshore)' : roles[Math.floor(Math.random() * roles.length)], // Pastikan Budi selalu Driller
      phone: phone,
      pfi: pfi, 
      prs: prs, 
      wwi: wwi, 
...
export default function App() {
  ...
  // Mobile Stack Navigation States (Modals)
  ...
  // Dapatkan data spesifik untuk pengguna role 'pekerja'
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setWorkers(prevWorkers => 
        prevWorkers.map(w => {
          // Jika mode privasi nyala, data pekerja yang login tidak diupdate (dibekukan/disembunyikan)
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    const user = usersDB.find(u => u.email === authForm.email && u.password === authForm.password);
    if (user) {
      setCurrentUser(user);
      setActiveTab('home');
        ...
        <div className="flex-1 bg-slate-50 flex flex-col p-6 overflow-y-auto pt-12">
          ...
          <div className="flex-1">
            ...
            <form onSubmit={authMode === 'login' ? handleLogin : handleRegister} className="space-y-4">
              {authMode === 'register' && (
                <div>
                  <div className="relative">
                    ...
                    <input type="text" name="name" value={authForm.name} onChange={handleAuthInputChange} className="w-full pl-10 pr-3 py-3 text-sm border border-slate-300 rounded-xl focus:outline-none focus:border-blue-500 bg-white" placeholder="Nama Lengkap" />
                  </div>
                </div>
              )}
              <div>
              ...
              {authMode === 'register' && (
                <div>
                  <select name="role" value={authForm.role} onChange={handleAuthInputChange} className="w-full px-3 py-3 text-sm border border-slate-300 rounded-xl focus:outline-none focus:border-blue-500 bg-white text-slate-700">
                    <option value="pekerja">Pekerja Lapangan / Operator</option>
                    <option value="supervisor">Supervisor Operasi</option>
                    <option value="medis">Tim Medis / HSSE</option>
                  </select>
                </div>
              )}
          ...
          {authMode === 'login' && (
            <div className="mt-8 p-4 bg-slate-100 rounded-xl text-xs text-slate-600 border border-slate-200">
              <p className="font-bold mb-2">Akun Uji Coba (Pass: password):</p>
              <ul className="space-y-1">
                <li>• budi@wellguard.com <span className="font-bold text-blue-600">(Pekerja)</span></li>
                <li>• medis@wellguard.com (HSSE)</li>
                <li>• spv@wellguard.com (SPV)</li>
              </ul>
            </div>
          )}
        </div>
  ...
  // --- DATABASE TAB (Dapat Diakses Admin/Medis) ---
  const DatabaseTab = () => {
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-slate-50 pb-20 flex flex-col h-full">
         <TopBar title="Database Rekam Jejak" />
  ...
  // ==========================================
  // VIEWS: KHUSUS PEKERJA (WORKER EXCLUSIVE)
  // ==========================================

  const HomeTabWorker = () => {
    if(!myWorkerData) return null;
    const w = myWorkerData;

    return (
      <div className="flex-1 overflow-y-auto bg-slate-50 pb-20">
        <TopBar title="My Bio-Sync" />
  ...
  const CoachTabWorker = () => {
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-slate-50 pb-20">
        <TopBar title="AI Self-Care Coach" />
        <div className="p-4 space-y-4">
  ...
  const PrivacyTabWorker = () => {
    return (
      <div className="flex-1 overflow-y-auto bg-slate-50 pb-20">
        <TopBar title="Perisai Privasi" />
        <div className="p-4 space-y-4">
  ...
  // ==========================================
  // FULL SCREEN PUSHED VIEWS (MODALS)
  // ==========================================

  // 1. Worker Live Detail (Biometric) - DENGAN INOVASI 3D DIGITAL TWIN
  const WorkerDetailView = () => {
    if (!activeDetailWorker) return null;
    const w = activeDetailWorker;
    
    // Logika AI penentu bagian tubuh mana yang menyala merah
    const isHeadAnomalous = w.prs !== 'Stabil' || w.sensors.gsr >= 2.0; // Stres/Mental
    const isChestAnomalous = w.sensors.bpm >= 90; // Jantung (BPM)
    const isLungsAnomalous = w.sensors.spo2 <= 95; // Paru-paru (Oksigen)
    const isBodyAnomalous = w.sensors.temp >= 37.5; // Suhu Seluruh Tubuh/Kulit
    ...
    return (
      <div className="absolute inset-0 bg-slate-900 z-20 flex flex-col animate-in slide-in-from-right-full duration-200 text-white">
        ...
        <div className="flex-1 overflow-y-auto pb-6">
           ...
           {/* Visualisasi 3D Digital Twin (Hologram) */}
           <div className="relative h-64 w-full flex items-center justify-center bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 to-slate-900 overflow-hidden border-b border-slate-800">
              ...
              <svg viewBox="0 0 100 200" className="h-56 w-auto relative z-10 drop-shadow-xl">
                 ...
                 {/* Kepala (Mental & Stres) */}
                 <g className={getPartClass(isHeadAnomalous)}>
                   <path d="M 50 10 C 42 10 38 18 38 25 C 38 32 45 35 50 38 C 55 35 62 32 62 25 C 62 18 58 10 50 10 Z" />
                 </g>
                 
                 {/* Leher */}
                 <rect x="46" y="38" width="8" height="6" className={getPartClass(isBodyAnomalous)} />
                 
                 {/* Bahu dan Lengan (Suhu) */}
                 <g className={getPartClass(isBodyAnomalous)}>
                   <path d="M 30 45 Q 46 42 50 44 Q 54 42 70 45 L 75 90 C 75 95 65 95 65 90 L 62 55 L 50 50 L 38 55 L 35 90 C 35 95 25 95 25 90 Z" />
                 </g>

                 {/* Paru-paru (SpO2) */}
                 <g className={getPartClass(isLungsAnomalous)}>
                   <path d="M 38 55 C 35 65 42 75 48 75 C 49 70 51 70 52 75 C 58 75 65 65 62 55 C 55 58 45 58 38 55 Z" />
                 </g>
                 ...
                 {/* Pinggang/Perut (Suhu) */}
                 <g className={getPartClass(isBodyAnomalous)}>
                   <path d="M 40 76 L 60 76 L 58 105 L 42 105 Z" />
                 </g>
           ...
           {/* Detail Angka dan Rekomendasi di bawahnya */}
           <div className="p-4 space-y-4 mt-2">
              ...
              {/* Biometric Data Grid (Hanya Angka) */}
              ...
              {/* Status Psikologis dari Teks (NLP) */}
  ...
  // 2. Database History Detail
  const DatabaseDetailView = () => {
    if (!activeDbWorker) return null;
    const w = activeDbWorker;
    return (
  ...
  const ContactDetailView = () => {
    ...
    if (!activeContactWorker) return null;
    const w = activeContactWorker;
  ...
  // 4. Alerts List View (from Dashboard)
  ...
  const BottomNav = () => {
    ...
    return (
      <div className="absolute bottom-0 inset-x-0 h-16 bg-white border-t border-slate-200 flex justify-around items-center px-2 z-10 rounded-b-[2.5rem]">
         ...
         {/* Kondisi Navigasi Berdasarkan Role */}
         {isWorker ? (
            <>
  ...
  return (
    <MobileFrame>
       {/* Routing Sederhana Berdasarkan Role */}
       {currentUser.role === 'pekerja' ? (
          <>
            {activeTab === 'home' && <HomeTabWorker />}
            {activeTab === 'aicoach' && <CoachTabWorker />}
            {activeTab === 'privacy' && <PrivacyTabWorker />}
            {activeTab === 'profile' && <ProfileTab />}
          </>
       ) : (
          <>
            {activeTab === 'home' && <HomeTabAdmin />}
       ...
       {/* Jendela Pop-up Rincian Lanjutan (Overlays) */}
       <AlertsListView />
       <WorkerDetailView />
       <DatabaseDetailView />
       <ContactDetailView />
